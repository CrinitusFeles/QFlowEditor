...
class LoopEndGNode(QDMGraphicsNode):
    ...
    def paint(self, painter, option, widget=None):
        ...
        path_content.addPolygon(QPolygonF( [
            ...
            QPointF(self.width, self.height - self.height / 3),
            QPointF(self.width / 2 + self.content_width / 2, self.height),
            QPointF(self.width / 2 - self.content_width / 2, self.height),
            QPointF(0, self.height - self.height / 3),
        ] ))
        # path_content.lineTo(0, self.height / 2)

        painter.setPen(Qt.PenStyle.NoPen)
        painter.setBrush(QBrush(QColor(colors.loop_end_bg)))
